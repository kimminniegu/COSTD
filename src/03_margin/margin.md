...
- 연동 결과: 원가 합계 3,173 × 1.03 = **3,268원**, 209카톤 · **3.14 CBM**. 기준 환율은 바꾸지 않습니다(상단 `margin-fx` 값 유지). 기준 환율 1,400원이면 FOB **$3.53**, 1,351.1원(TTB)이면 $3.66.
- 오른쪽 탭 맞춤:
  - 역제안 **바이어 목표가**(`margin-target`) = 연동 후 견적 단가 × (1 − 5%), 센트 반올림 → 1,400원 기준 **$3.35** · `영업마진 양보` + `팀장 전결 가능` · 영업마진 15.6%. 단가 기준이라 기준 환율과 관계없이 같은 판정 구간(마진율 20/15% 기준 영업마진 약 15.8%)에서 시작합니다. (브리프의 바이어 타깃 $2.35는 손익분기 $2.37보다 낮아 시연값으로 쓰지 않음)
...
- **① 상단 Card — 입력과 결과를 한 Card 안에서**
  ...
  3. 마지노선 환율 칩(구 '버틸 수 있는 환율'): `손익분기 940원` · `최소 마진(15%) 1,316원` · `목표 마진(20%) 1,398원`. 색 점은 판정 구간 색, 견적 환율에서 이미 미달이면 숫자를 `--color-danger`로 표시. 계약 단가는 센트 단위라 반올림 폭(±$0.005, 환율로 약 2원) 안의 차이는 달성으로 봅니다 — 판정 Badge·칩·방어 단가 `현재 단가로 달성`이 같은 기준.
