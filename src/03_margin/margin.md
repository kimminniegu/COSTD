...
| 2.31 | 2026-09-26 | 좌측 입력 Card: Card가 내용보다 길 때 남는 높이를 물류비 ↔ 현재 견적 요약 사이에 몰지 않고 견적 조건 여백·행 간격·라벨↔입력·구분선 위아래·그룹 사이·제목↔접이식·요약 위 간격에 **고르게 나눔**(`fitAside` → `--margin-aside-flex`, 가중치 합 6, 한 단위 최대 40px — 더 남으면 요약 위로). 요소·순서는 그대로, 브라우저 높이 약 750px 이하는 기존 촘촘한 간격 유지 | C |
...
- 입력 Card 높이: margin.js `fitAside()`가 **처음 화면 기준**(`innerHeight − Card 시작 위치 − 24px`)으로 높이를 정해 바닥이 화면 아래 끝에 닿게 하고, 스크롤해도 늘리지 않아 제조원가·물류비·현재 견적 요약 사이 간격이 그대로 유지됩니다. 토글을 모두 접은 기본 상태에서는 Card 안에 스크롤이 생기지 않도록 **내용 높이까지** 늘립니다(견적 조건 여백 `20px / 12px`, 라벨↔입력 4px, 제조원가·물류비는 그룹 사이 8px · 제목↔접이식 4px · 접이식 머리 위아래 6px로 촘촘하게 해 내용 높이 약 525px). 토글을 펼쳐 내용이 그 높이보다 길면 스크롤 중 화면에 남는 만큼 **내용 높이까지만** 늘립니다. Card가 내용보다 길면 남는 높이를 요약 박스 위(`margin-top: auto`)에 몰지 않고 `--margin-aside-flex`(한 단위 = 남는 높이 ÷ 6, 최대 40px)로 간격에 나눕니다 — 견적 조건 위 여백 0.25 · 제목↔1행↔2행 0.5×2 · 라벨↔입력 0.125×2 · 구분선 위 1 · 구분선 아래 1 · 제조원가↔물류비 1 · 제목↔접이식 0.25×2 · 물류비↔요약 1 (단위 최대를 넘는 나머지만 요약 위로). resize·scroll·접이식 toggle 때 다시 계산하고, 1열 배치(position: static)에서는 해제합니다.
