...
| **역제안 분석** | 바이어 목표가 판정(4단계) + **영업 승인 가이드라인(R&R) Badge**, 가격 위치 Gauge(손익분기·최대 양보가·최소 마진가·목표 마진가), **사양 변경(VE) 추천**(1차·물류 마진 조정 / 불가 판정 시), 항목별 마진 직접 조정, 대응 방안 카드 |
| **수량별 단가** | MOQ·MOQ 미만 정책(소량 할증 / 주문 불가), 수량별 단가 막대 차트, 편집 가능한 가격표, 바이어용 가격표 텍스트 복사, **영문 이메일 제안문 복사** |
...
| 역제안 분석 › VE | 코팅/후가공 생략 · 단상자 평량/단일도수 · 부자재 사급 전환 | `margin-ve-coat` / `-box` / `-sagup` | 해제 (입력에서 이미 사급이면 사급 전환은 비활성) |
...
목표가 t(USD)를 원화 FOB 기준 T로 되돌린 뒤(CIF 보험·해상운임 제외) 네 기준가와 비교합니다. 기준가·판정·Gauge는 `priceLines()` / `zoneOf()` / `drawGauge()`로 묶여 있어 VE 미리보기도 같은 로직을 씁니다.
...
**영업 승인 가이드라인 (R&R)** — 목표가에서 남는 영업마진 `m = 1 − P2 / T`(판매가 대비)로 판단합니다. 판정 박스, 마진 직접 조정 결과(조정안의 영업마진), VE 미리보기(절감 후 영업마진)에 같은 Badge가 붙습니다.
...
**사양 변경(VE) 추천** — 판정이 "1차·물류 마진 조정"(orange) 또는 "마진만으로 불가"(red)일 때만 Card를 보여줍니다. 체크 상태는 Card가 숨겨져도 유지됩니다.
...
```
r′ = forward(p, { pack: max(0, 부자재 원가 − 차감액), rates.pack: 사급 전환 ? 0 : 현재 })
P′ = priceLines(p, r′)        영업마진′ = 1 − r′.P2 / T        판정′ = zoneOf(t, P′)
```

- 결과 박스: "현재 판정 → 개선 판정" Badge + R&R Badge, 개당 공급가 절감액(`r.P2 − r′.P2`), 최소 마진가 변화, 주문 전체 절감액(× 수량). 아래에 절감 후 기준가로 다시 그린 Gauge를 보여줍니다.
- 아무것도 체크하지 않으면 안내 문구만 표시하고 Gauge는 비웁니다.
- 절감액은 약식 추정이므로 "공급사 확인 필요" 문구를 함께 표시합니다.
...
.margin-page#margin-app
 ...
 └─ .margin-layout (360px + 1fr, 본문 폭 ≤ 880px에서 1열)
     ...
     └─ .margin-results
          ...
          ├─ .tab-panel#margin-view-tier    : MOQ·요약 / 차트 / 가격표(가격표 복사 · 영문 이메일 제안문 복사 · 수량 추가)
...
- 접이식 CBM 추정은 별도 JS 없이 네이티브 `<details>`/`<summary>`를 쓰고 모양만 `.margin-drawer*`로 지정합니다. 표시·숨김이 필요한 영역(VE Card, 흡수 옵션, 완료 안내)은 `hidden` 속성으로 토글하고, class의 `display`에 덮이지 않도록 `.margin-page [hidden]`, `#margin-quote-modal [hidden]`을 지정합니다.
...
- [ ] 역제안: orange·red 판정에서만 VE Card가 보이고, 체크하면 개선 판정·Gauge·절감액이 즉시 바뀐다.
