...
1. **상단 Card — 바이어 목표가 위치 및 판정 구간**: 왼쪽 입력 묶음은 `라벨 | 입력칸` 2행(`margin-target` 바이어 목표가, `margin-item-floor` **1차 마진 양보 한계치 (항목별)**, 남은 폭을 채움), 오른쪽 종합 판정 박스(`#margin-verdict`)는 내용 폭 그대로(판정·R&R Badge 한 줄 · 영업마진 · 설명 한 줄) 고정해 한 줄에 놓고, 중간 폭 카드(`.margin-rtop` 안쪽 620~759px, 화면 약 1440px)에서도 같은 구조를 유지하려고 긴 라벨을 `1차 마진 양보 / 한계치 (항목별)` 두 줄(`.margin-rtop__br`)로, 입력칸을 좁게(안쪽 여백 축소) 합니다. 이보다 좁은 카드(화면 약 1280px 이하)에서는 판정 박스가 입력 아래 줄로 내려갑니다. 그 아래 구분선 → '가격 위치' 제목 + 판정 구간 범례 → Gauge(`#margin-gauge`) → 시뮬레이션 적용 안내(`#margin-gauge-note`). Gauge 막대는 **손익분기에서 시작**하고 색 구간 경계가 기준가 눈금(손익분기·최대 양보가·최소 마진가·목표 마진가)과 정확히 겹칩니다(눈금·핀을 막대와 같은 상자 기준 %로 배치). 목표가가 손익분기보다 낮으면 그만큼 회색(`.is-loss`) 구간을 왼쪽에 붙입니다. 바이어 목표가는 말풍선 + 꼬리 + 막대 위 점(`.margin-gauge__dot`)으로 표시하고, 막대 양 끝의 눈금·말풍선은 안쪽으로 정렬합니다. 기준가 라벨은 막대 폭 기준 84px 안으로 붙을 때만 두 번째 줄(`.has-row2`)로 내립니다. 판정 문구·R&R Badge·부족 금액은 이 박스 한 곳에만 표시합니다.
...
.margin-page#margin-app
 ...
 └─ .margin-layout (360px + 1fr, 본문 폭 ≤ 880px에서 1열)
     ...
     └─ .margin-results
          ...
          ├─ .tab-panel#margin-view-reverse : ① .card(.margin-rtop: [입력(라벨 | 입력칸 2행, title 툴팁) | #margin-verdict(내용 폭)] → 구분선 → .margin-rtop__gauge('가격 위치' 제목 + 범례 한 줄 → #margin-gauge → 적용 후 안내)) / ② 대응 방안 .card(.margin-options) / ③ .margin-rmid(VE .card | 마진 직접 조정 .card)
