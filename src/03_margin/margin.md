...
| 탭 | 기능 |
|---|---|
| 좌측 입력 › **현재 견적 요약** | 입력 Card 맨 아래(`#margin-quick`, `margin-top: auto`): 인코텀즈 단가 · 주문 총액 · 할인 후 영업마진(최소 미만이면 danger). 어느 탭에서든 확인 |
...
| 역제안 분석 | 바이어 목표가 / 1차 마진 양보 한계치 (항목별) | `margin-target` / `margin-item-floor` | $1.95 / 5% |
...
1. **상단 Card — 바이어 목표가 위치 및 판정 구간**: 왼쪽 입력(`margin-target` 바이어 목표가, `margin-item-floor` **1차 마진 양보 한계치 (항목별)**, `라벨 | 입력` 2행), 오른쪽 종합 판정 박스(`#margin-verdict`), 아래 Gauge(`#margin-gauge`), 그 아래 한 줄에 판정 구간 범례와 시뮬레이션 적용 안내(`#margin-gauge-note`). Gauge 아래 여백은 기준가 라벨이 두 줄로 겹칠 때만(`.has-row2`) 늘립니다. 판정 문구·R&R Badge·부족 금액은 이 박스 한 곳에만 표시합니다.
