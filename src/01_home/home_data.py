...
def init(db_path: Path) -> None:
    ...
    with _conn() as c:
        c.executescript(
            ...
            CREATE TABLE IF NOT EXISTS world_trade (
                year INTEGER, hs4 TEXT, reporter INTEGER, reporter_iso TEXT, reporter_name TEXT,
                partner TEXT, value_usd REAL, fetched_at TEXT,
                PRIMARY KEY (year, hs4, reporter, partner)
            );
            """
        )
...
# ---------------------------------------------------------------------------
# 5-10. 세계 수입시장 — UN Comtrade (연간, 나라별 화장품 수입액과 그중 한국산)
#   키 없이 쓰는 preview 는 한 호출에 1~3분 걸리므로 백그라운드로 받아 7일 캐시. 키(COMTRADE_API_KEY)가 있으면 빠른 정식 endpoint 사용.
# ---------------------------------------------------------------------------
COMTRADE_KEY = os.getenv("COMTRADE_API_KEY") or None
...
COUNTRY_KO = {"CN": "중국", "US": "미국", "JP": "일본", "HK": "홍콩", "VN": "베트남", "RU": "러시아", "TW": "대만", "TH": "태국",
              ...
              "ES": "스페인", "BE": "벨기에", "CH": "스위스", "MX": "멕시코", "BR": "브라질", "IN": "인도", "TR": "튀르키예", "KZ": "카자흐스탄",
              "UA": "우크라이나", "CL": "칠레", "SE": "스웨덴", "AT": "오스트리아", "CZ": "체코", "IE": "아일랜드", "IL": "이스라엘", "NZ": "뉴질랜드",
              "MO": "마카오", "KW": "쿠웨이트", "QA": "카타르", "PT": "포르투갈", "DK": "덴마크", "NO": "노르웨이", "FI": "핀란드", "HU": "헝가리",
              "RO": "루마니아", "GR": "그리스", "ZA": "남아프리카공화국", "EG": "이집트", "AR": "아르헨티나", "CO": "콜롬비아", "PE": "페루", "KR": "한국"}


def _comtrade_get(params: dict) -> list[dict]:
    url, headers = (COMTRADE_DATA, {**HEADERS, "Ocp-Apim-Subscription-Key": COMTRADE_KEY}) if COMTRADE_KEY else (COMTRADE_PREVIEW, HEADERS)
    res = requests.get(url, params=params, headers=headers, timeout=300)
    res.raise_for_status()
    return res.json().get("data", []) or []


def _comtrade_reporters() -> dict[int, dict]:
    """국가 코드표(숫자 → ISO2·영문명). meta 에 30일 캐시"""
    cached = _meta_get("comtrade_reporters")
    if cached and not _is_stale("comtrade_reporters_at", 30 * 86400):
        return {int(k): v for k, v in json.loads(cached).items()}
    res = requests.get(COMTRADE_REPORTERS, headers=HEADERS, timeout=60)
    res.raise_for_status()
    items = res.json().get("results", [])
    table = {int(x["id"]): {"iso": x.get("reporterCodeIsoAlpha2") or "", "name": x.get("text") or ""} for x in items if not x.get("isGroup")}
    _meta_set("comtrade_reporters", json.dumps(table, ensure_ascii=False))
    _meta_set("comtrade_reporters_at", _iso(_now()))
    return table


def refresh_world(hs4: str) -> None:
    """hs4 의 최신 연도(작년, 없으면 재작년)에 대해 각국 총수입(partner 0)과 한국산 수입(partner 410)을 받아 저장"""
    now = _now()
    _meta_set(f"world_last_try:{hs4}", _iso(now))
    try:
        reporters = _comtrade_reporters()
    except Exception as err:  # noqa: BLE001
        log.warning("Comtrade 국가표 실패: %s", err)
        reporters = {}
    common = {"cmdCode": hs4, "flowCode": "M", "partner2Code": "0", "motCode": "0", "customsCode": "C00"}
    for year in (now.year - 1, now.year - 2):
        # 한 번의 호출로 전 세계(0)와 한국(410) 두 partner 를 함께 받아 호출 수를 절반으로 (공개 preview 는 시간당 제한이 있어 500 이 자주 남)
        rows: list[dict] = []
        for attempt in (1, 2):
            try:
                rows = _comtrade_get(common | {"period": str(year), "partnerCode": f"0,{KOREA_CODE}"})
                break
            except Exception as err:  # noqa: BLE001
                log.warning("Comtrade 조회 실패(%s %s, %d회): %s", hs4, year, attempt, err)
                if attempt == 1:
                    time.sleep(60)
        def _pc(r):   # partnerCode 0(전 세계)이 falsy 라 `or` 로 처리하면 안 됨
            v = r.get("partnerCode")
            return int(v) if v not in (None, "") else -1
        world = [r for r in rows if _pc(r) == 0]
        korea = [r for r in rows if _pc(r) == KOREA_CODE]
        if not world:
            continue
        best: dict[tuple[int, str], float] = {}
        for partner, rows in (("world", world), ("kor", korea)):
            for r in rows:
                rc = int(r.get("reporterCode") or 0)
                if not rc or rc == KOREA_CODE and partner == "kor":
                    continue
                v = float(r.get("primaryValue") or 0)
                best[(rc, partner)] = max(best.get((rc, partner), 0.0), v)   # 세부 행이 섞여도 총계(최대값) 채택
        with _conn() as c:
            c.execute("DELETE FROM world_trade WHERE year=? AND hs4=?", (year, hs4))
            names = {}   # 응답의 reporterISO/Desc 를 우선, 없으면 코드표
            for r in world + korea:
                rc = int(r.get("reporterCode") or 0)
                if rc and rc not in names:
                    ref = reporters.get(rc, {})
                    iso3 = r.get("reporterISO") or ""
                    names[rc] = (ref.get("iso") or "", r.get("reporterDesc") or ref.get("name") or str(rc), iso3)
            c.executemany(
                "INSERT OR REPLACE INTO world_trade (year, hs4, reporter, reporter_iso, reporter_name, partner, value_usd, fetched_at) VALUES (?,?,?,?,?,?,?,?)",
                [(year, hs4, rc, names.get(rc, ("", "", ""))[0], names.get(rc, ("", str(rc), ""))[1], partner, v, _iso(now))
                 for (rc, partner), v in best.items()],
            )
        _meta_set(f"world_has_korea:{hs4}:{year}", "1" if korea else "0")
        _meta_set(f"world_year:{hs4}", str(year))
        _meta_set(f"world_fetched_at:{hs4}", _iso(now))
        log.info("Comtrade 수집 %s %s: 국가 %d", hs4, year, len({k[0] for k in best}))
        return
    log.warning("Comtrade 자료 없음: %s", hs4)


def kick_world(hs4: str) -> bool:
    """오래됐으면 백그라운드 수집 시작. 시작했거나 진행 중이면 True"""
    key = f"world:{hs4}"
    with _lock:
    ...
    threading.Thread(target=_run, name=f"home-world-{hs4}", daemon=True).start()
    return True


def get_world(hs4: str) -> dict:
    """/api/home/world — 세계 수입시장 상위국과 한국산 비중 (5-10)"""
    hs4 = (hs4 or "3304")[:4]
    if hs4 not in TRADE_ALL_CODES:
        hs4 = "3304"
    refreshing = kick_world(hs4)
    year = _meta_get(f"world_year:{hs4}")
    out = {"ok": False, "hs4": hs4, "year": int(year) if year else None, "refreshing": refreshing,
           "source": "UN Comtrade", "source_url": "https://comtradeplus.un.org/TradeFlow", "importers": []}
    if not year:
        out["message"] = "UN Comtrade에서 받아오는 중이에요. 처음에는 몇 분 걸려요" if refreshing else "세계 수입시장 자료를 아직 받지 못했어요"
        return out
    with _conn() as c:
        rows = [dict(r) for r in c.execute("SELECT reporter, reporter_iso, reporter_name, partner, value_usd FROM world_trade WHERE year=? AND hs4=?", (int(year), hs4))]
    by: dict[int, dict] = {}
    for r in rows:
        d = by.setdefault(r["reporter"], {"code": r["reporter"], "iso": r["reporter_iso"], "name_en": r["reporter_name"], "world": 0.0, "kor": 0.0})
        d[r["partner"]] = float(r["value_usd"])
    importers = sorted((d for d in by.values() if d["code"] != KOREA_CODE), key=lambda d: -d["world"])   # 한국 자체 수입은 제외
    world_total = sum(d["world"] for d in importers) or 1.0
    kor_total = sum(d["kor"] for d in importers)
    has_korea = _meta_get(f"world_has_korea:{hs4}:{year}") == "1"
    top = []
    for i, d in enumerate(importers[:WORLD_TOP], 1):
        top.append({
            "rank": i, "iso": d["iso"], "name": COUNTRY_KO.get(d["iso"], d["name_en"]), "name_en": d["name_en"],
            "imports_musd": f"{d['world'] / 1e6:,.0f}", "from_korea_musd": f"{d['kor'] / 1e6:,.1f}",
            "korea_share": round(d["kor"] / d["world"] * 100, 1) if (has_korea and d["world"]) else None,
            "world_share": round(d["world"] / world_total * 100, 1),
            "bar": round(d["world"] / importers[0]["world"] * 100, 1) if importers and importers[0]["world"] else 0,
        })
    return out | {
        "ok": True, "importers": top, "reporters": len(importers),
        "world_total_musd": f"{world_total / 1e6:,.0f}", "korea_total_musd": f"{kor_total / 1e6:,.0f}",
        "korea_share_total": round(kor_total / world_total * 100, 1) if has_korea else None,
        "has_korea": has_korea,
        "fetched_at": _meta_get(f"world_fetched_at:{hs4}"),
    }
