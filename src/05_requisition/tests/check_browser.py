...
try {
  ...
  enter('sample_request_type', '신규 샘플');
  ...
  assert(pdf.querySelector('.document-meta').textContent.includes('사내'), 'internal retention label');
  ...
  for (let i = 0; i < 20 && by('pdf-external').disabled; i++) await new Promise((resolve) => setTimeout(resolve, 10));
  by('pdf-external').click();
  for (let i = 0; i < 20 && !document.querySelector('.requisition-print-frame')?.srcdoc.includes('외부 전달용'); i++) await new Promise((resolve) => setTimeout(resolve, 10));
  const externalFrame = document.querySelector('.requisition-print-frame');
  const externalPdf = new DOMParser().parseFromString(externalFrame.srcdoc, 'text/html');
  assert(externalPdf.querySelector('.document-type').textContent === '외부 전달용', 'external PDF label');
  assert(!externalPdf.body.textContent.includes('Browser customer'), 'external PDF excludes customer');
  assert(!externalPdf.body.textContent.includes('작성 방식'), 'external PDF excludes creation method');
  assert(!externalPdf.querySelector('.review-signoff'), 'external PDF excludes internal review');
